...
export default {
  ...
  data() {
    return {
      ...
      isActiveDown: this.message.usersDisliked?.includes(JSON.parse(localStorage.getItem("user")).userId) || false,
      isActiveUp: this.message.usersLiked?.includes(JSON.parse(localStorage.getItem("user")).userId) || false,
    };
  },
  computed: {},
  methods: {
